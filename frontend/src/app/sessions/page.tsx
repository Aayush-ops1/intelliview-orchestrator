...
import type { InterviewSession, TaskPriority } from "@/lib/types";

const TABS = ["active", "completed", "failed"] as const;
...
export default function SessionsPage() {
  ...
  const active = useSWR<{ count: number; sessions: InterviewSession[] }>("/active-sessions", { refreshInterval: 2000 });
  const completed = useSWR<{ count: number; sessions: InterviewSession[] }>("/completed-sessions?limit=100", { refreshInterval: 10000 });
  const failed = useSWR<{ count: number; sessions: InterviewSession[] }>("/failed-sessions?limit=100", { refreshInterval: 10000 });
      ...
      <Card>
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            <SearchInput value={search} onChange={setSearch} placeholder="Filter by id or candidate…" className="w-64" />
            <button
              onClick={() => data.mutate()}
        ...
        {data.error ? (
          ...
          <EmptyState title={search ? "No matches" : `No ${tab} sessions`} description={search ? "Try a different search term." : "Sessions matching this state will appear here."} />
        ) : (
              ...
              <tbody>
                {filtered.map((s) => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-4"><Pipeline current={s.status} /></td>
...
function StartInterviewForm({ disabled }: { disabled: boolean }) {
  ...
  return (
    <Card title="Start interview" description="Enqueue a new session for processing.">
      ...
      {disabled && <div className="mt-2 text-xs text-amber-400">Set an API token in the top bar to start sessions.</div>}
    </Card>
  );
}
